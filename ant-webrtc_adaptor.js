...
function WebRTCAdaptor(initialValues)
{
	...
	thiz.isIOSApp = window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.iOSHandler;
	...
	this.mergeAudio = function (stream, micStream) {
		...
		var tracks = audioDestionation.stream.getAudioTracks();
		...
		if (audioConstraint != "undefined" && audioConstraint != false) {
			...
			.then(function(audioStream) {
				...
				if (thiz.publishMode == "screen") {
					var mergedStream = thiz.mergeAudio(stream, audioStream);
					thiz.switchDesktopSource(mergedStream, streamId, mediaConstraints, onended, true);
				}
				else if (thiz.publishMode == "screen+camera") {
					var mergedStream = thiz.mergeAudio(audioStream, stream);
					if (audioTrack.length > 0) {
						stream.removeTrack(audioTrack[0]);
					}
					thiz.switchDesktopwithCameraSource(stream, streamId, mergedStream, onended);
				}
				else {
					if (audioTrack.length > 0) {
						stream.removeTrack(audioTrack[0]);
	...
	if (!this.isPlayMode && typeof thiz.mediaConstraints != "undefined" && this.localStream == null)  
	{
		...
		{   
			...
			if (thiz.mediaConstraints.audio.mandatory) 
			{
				...
				var mc = thiz.isIOSApp ? {audio:false, video:false} : {audio:true, video:false}
				navigator.mediaDevices.getUserMedia(mc).then(function(micStream){
